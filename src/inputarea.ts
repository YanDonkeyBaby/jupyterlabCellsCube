...
export class InputArea extends Widget {
  ...
  constructor(options: InputArea.IOptions) {
    ...
    console.log('InputArea===editorOptions==',editorOptions)
  ...
  dispose() {
    ...
    this._editor = null!;
    this._rendered = null!;
    this._addedWiget = null!;
    super.dispose();
  }
  /**
   * 获取PanelLayout中增加的widget
   */
  get addedWidget(): Widget {
    return this._addedWiget;
  }
  /**
   * 在PanelLayout中增加一个widget
   */
  addWidget(widget:Widget):void{
    if(!this._addedWiget){
      const layout = this.layout as PanelLayout;
      this._addedWiget = widget;
      layout.addWidget(widget);
    }
  }
